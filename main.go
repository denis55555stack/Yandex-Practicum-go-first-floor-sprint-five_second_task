...
// distance возвращает дистанцию, которую преодолел пользователь.ß
// Формула расчета:
...
func (r Running) Calories() float64 {
	...
	calories := ((CaloriesMeanSpeedMultiplier*avgSpeed + CaloriesMeanSpeedShift) * r.Weight / MInKm * r.Duration.Hours() * MinInHours)
	return calories
...
func (w Walking) Calories() float64 {
	...
	hInM := w.Height / CmInM       // Перевод роста из сантиметров в метры.
	calories := ((CaloriesWeightMultiplier*w.Weight + (math.Pow(mInSec, power)/hInM)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours)
	return calories
}
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	swimmingAvgSpeed := s.meanSpeed()
	calories := (swimmingAvgSpeed + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * s.Duration.Hours()
	return calories
}
